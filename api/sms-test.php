...
/* وضعیت روشن/خاموش — مهم‌ترین ردیف این صفحه. وقتی خاموش است، هیچ
   پیامکی واقعاً فرستاده نمی‌شود و کد تأیید روی صفحه‌ی سایت نشان داده
   می‌شود (حالت آزمایش قبل از راه‌اندازی). */
$smsOn = !empty($smsCfg['enabled']);
$rows[] = row(
    'ارسال پیامک',
    $smsOn,
    $smsOn
        ? 'روشن است — پیامک‌ها واقعاً فرستاده می‌شوند و اعتبار خرج می‌شود'
        : 'خاموش است (حالت آزمایش) — هیچ پیامکی فرستاده نمی‌شود، '
          . 'هیچ اعتباری خرج نمی‌شود، و کد تأیید موقع ورود روی صفحه‌ی سایت نوشته می‌شود'
);
...
{
    /* عمداً بدترین حالت‌ها گذاشته شده تا سقف طول پیامک دیده شود:
       بلندترین نام خدمت، بلندترین تاریخ («چهارشنبه ۱۰ اردیبهشت»)
       و یک علت ردِ کوتاه. */
    return [
        'code'    => '1234',
        'date'    => 'چهارشنبه ۱۰ اردیبهشت',
        'time'    => '۱۶:۳۰',
...
<?php if ($sendResult !== null && !empty($sendResult['skipped'])): ?>
  <div class="box box--no">
    <strong>⚠ پیامکی فرستاده نشد — ارسال پیامک خاموش است</strong><br>
    نتیجه «موفق» است، ولی این یعنی مسیر کد بدون خطا اجرا شد، نه اینکه
    پیامک رسیده باشد. برای اینکه واقعاً فرستاده شود، در
    <code>api/config.php</code> مقدار <code>'enabled' =&gt; true</code>
    را بگذارید و این تست را دوباره بگیرید.<br>
    (خاموش بودن عمدی است: قبل از راه‌اندازی، ورود با کدِ روی صفحه انجام
    می‌شود و اعتبار خرج نمی‌شود.)
  </div>
<?php elseif ($sendResult !== null): ?>
