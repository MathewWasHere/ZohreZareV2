...
final class Config
{
    ...
    public static function load(): void
    {
        ...
        /* اگر config.php خطای نگارشی داشته باشد — مثلاً متنی که داخل
           کوتیشن نگذاشته‌اید، یا «کوتیشن فارسی» به‌جای ' — خطای PHP کل
           درخواست را با صفحه‌ی سفید رد می‌کند و از بیرون هیچ سرنخی
           نیست. این‌جا خطایش را می‌گیریم تا هم در لاگ هاست بیفتد و هم
           شماره‌ی خطش به بیرون گفته شود.

           متن خطای PHP عمداً چاپ نمی‌شود: وقتی خودِ خط رمز عبور خراب
           باشد، آن پیام ممکن است تکه‌ای از رمز را داخلش داشته باشد. */
        try {
            $user = require $file;
        } catch (Throwable $e) {
            ...
            Http::fail(
                503,
                'فایل api/config.php خطای نگارشی دارد — حدود خط ' . $e->getLine() . '. '
                . '(خط گزارش‌شده گاهی چند خط بعد از اشتباه واقعی است؛ '
                . 'کوتیشنِ بازمانده آخرِ فایل گزارش می‌شود.) '
                . 'پیام دقیق خطا در error_log هاست ثبت شد.'
            );
        }
